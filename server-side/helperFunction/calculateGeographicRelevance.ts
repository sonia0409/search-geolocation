...
function calculateGeographicalRelevance(data: IData[], userData: IUserData){
    if(!data.length) return [];
    if(!userData.latitude || !userData.longitude) return "Invalid latitude or longitude values";
...
function getDistance(userLat:number, userLon:number, geoLat:number, geoLon:number): number{
...
function calculateConfidenceScore(lat1:number,lon1:number,lat2:number,lon2:number, maxDistance: number):number{
...
export default calculateGeographicalRelevance;
